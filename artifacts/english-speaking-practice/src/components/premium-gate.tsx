...
export function PremiumGate({ children, featureName = "This feature" }: { children: React.ReactNode, featureName?: string }) {
  ...
  return (
    <div className="flex h-full min-h-[60vh] flex-col items-center justify-center p-6 text-center">
      ...
      <h2 className="mb-2 font-serif text-3xl font-medium tracking-tight text-foreground">{featureName} is Premium</h2>
      ...
      <Link 
        href="/pricing" 
        className="flex items-center gap-2 rounded-xl bg-accent px-6 py-3.5 text-sm font-semibold text-accent-foreground shadow-md transition-all hover:scale-[1.02] active:scale-95"
      >
        <Sparkles size={18} />
        View Premium Plans
      </Link>
